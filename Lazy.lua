...
windower.register_event('addon command', function (...)
	...
	elseif args[1] == "show" then
		...
		log("Jiggle: "..tostring(jiggle).." (every "..jiggle_interval.."s)")
	...
	elseif args[1] == "jiggle" then
		if args[2] then
			jiggle = args[2]
			jiggle_timer = 0
			log("Jiggle set to " .. jiggle)
		else
			jiggle = false
			log("Jiggle disabled")
		end
	...
	elseif args[1] == "jiggle_interval" then
		local n = tonumber(args[2])
		if n then
			jiggle_interval = math.max(5, n)
			log("Jiggle interval: " .. jiggle_interval .. "s")
		end
	end
end)
...
local function do_jiggle()
    windower.send_command('setkey ' .. jiggle .. ' down')
    coroutine.sleep(0.25)
    windower.send_command('setkey ' .. jiggle .. ' up')
end
...
function Engine()
    ...
    if jiggle then
        if jiggle_timer <= 0 then
            do_jiggle()
            jiggle_timer = jiggle_interval
        else
